...
- Resolution: Android status refreshes now revalidate every definite or uncertain activation against the current authorized-device list. Global-proxy state is promoted only after an exact owned-proxy readback and becomes cleanup-only when another setting replaces it; companion state verifies package presence, a block-scoped live VPN state, and any exactly owned ADB reverse mapping. Companion launches remain visibly pending until the VPN is observed, confirmed and no-tunnel ownership is journaled across restarts, and cleanup preserves externally replaced reverse mappings. Disconnects and ambiguous ADB results become explicit uncertainty instead of a false active interception, confirmed stopped companion sessions retain only necessary cleanup, and later reconnects can promote a still-valid activation again. Cleanup ownership remains available to Stop without being presented as live interception.
