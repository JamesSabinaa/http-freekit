...
- Resolution: The renderer now distinguishes staged mock drafts, changed open editors, and focused inline renames from unchanged or reverted work. Save All validates and stages a live editor before taking its upload snapshot. Reload, New Session, navigation, window close, Quit, Restart, and Restart to install all block on unsaved mock work; browsers use the standard leave-page prompt, Electron uses a native leave/stay prompt, and updater restarts run the same explicit renderer preflight before entering their platform-specific quit sequence.
- Regression coverage: `test/bug-081-mock-unload-guard.test.js` and `test/bug-081-electron-unload-confirmation.test.js` cover staged drafts, new, changed, reverted, and pre-blur editors, inline renames, Save All snapshot ordering, canceled/accepted unloads, Send persistence failure, Electron native confirmation, updater restart preflight, handler registration, and Electron's combined quit preflight.
