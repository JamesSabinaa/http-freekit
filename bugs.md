...
- Resolution: Update checks now use one owned in-flight state instead of a shared mutable boolean. Overlapping callers join the same upstream check, manual intent only promotes that state, a manual caller joining after the checking event receives feedback, and terminal attribution remains with the operation until its promise settles. Requests arriving after a terminal event wait for settlement and start a fresh check, preventing coalesced stale results from consuming a new request. Checks, prompts/downloads, and installer handoffs have distinct lifecycle-owned state and cannot overlap at the updater layer, whose shared error event carries no operation metadata. Install preflights retain their own generation callbacks, while stop/re-init invalidates queued work, pending dialogs, late status delivery, listeners, and IPC handlers.
- Regression coverage: `test/bug-085-updater-check-attribution.test.js` covers automatic-to-manual promotion, protection from automatic demotion, a single attributed rejection, a request in the result/settlement gap, attribution reset for a later scheduled check, separately owned check/install errors, both forms of late old-preflight settlement, replacement of a pending prompt across re-init, cancellation of queued checks on stop, and listener/late-settlement isolation.
