...
- Resolution: Pin and unpin actions now mutate the exact authoritative traffic lifecycle through the management API and broadcast a revisioned state change to every renderer. Clear retains only valid server-pinned parent rows, returns and broadcasts their full authoritative snapshots, and preserves one bounded generation token for each pinned pending lifecycle so later HTTP completions and WebSocket frames remain attached across repeated Clears. Traffic imports reject invalid pin types, frames cannot be pinned independently, and traffic dumps, request updates, Clear results, detail/search APIs, MCP, exports, and reloads all use server pin state.
- Regression coverage: `test/bug-076-pinned-clear.test.js` covers exact reused-ID pinning, validation and ambiguity rejection, API/search/export persistence, unpin-and-clear behavior, repeated clears of pending requests, post-Clear WebSocket frames, bounded generation state, invalid imported/frame pins, authoritative renderer reconstruction, confirmation/failure behavior, stale-event rejection, and broadcast/response idempotency.
