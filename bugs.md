...
- Status: **Fixed**.
- Resolution: WebSocket capture now retains RSV1 across complete and fragmented messages, parses the negotiated permessage-deflate direction and context parameters, and inflates text and binary payloads for display without changing relayed bytes. Inflation runs asynchronously behind bounded per-direction capture queues after raw relay writes, output is capped, and context takeover copies only its small sliding window instead of retaining decoded-message backing buffers. Proxy shutdown waits for queued capture and connection-finalization events, invalid reserved-bit use disables only capture, and undecodable compressed messages show an explicit marker instead of corrupt UTF-8.
