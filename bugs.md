...
- Status: **Fixed**.
- Resolution: Startup preserves an expiring CA on every platform so a trusted root is never silently overwritten before replacement trust is available. Expiry and migration state are exposed 30 days ahead, and replacement requires an explicit, crash-safe renewal scheduled for the next restart. Windows trust cleanup retries and human client-migration acknowledgement use separate durable state; upgrades migrate legacy replacement state into the acknowledgement journal, and post-publication marker-hardening failures retain visible committed state without aborting startup. TLS settings refresh on entry and both UI and non-Windows boot warnings remain until the replacement CA has been installed in manually configured clients and acknowledged.
