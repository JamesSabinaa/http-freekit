...
- Regression coverage: `test/bug-123-android-ca-lifecycle.test.js` verifies that production fallback activation never issues an ADB push, persists the no-manual-CA marker, migrates legacy recovery ownership conservatively, performs no cleanup before confirmation, refuses replacement that would erase the marker, and wires the API/renderer handshake.
...
- Regression coverage: `test/bug-162-browser-spawn-confirmation.test.js` covers pending state before `spawn`, pending state after `spawn` but before stability confirmation, successful stable activation, pre-spawn errors, post-spawn exit/signal failures, managed-profile cleanup, surviving-descendant adoption, inspection-unknown ownership retention, its manager broadcast and final cross-client inactive broadcast, UI error refresh, later launcher errors, and temporary-listener cleanup for isolated and Global Chrome.
...
- Regression coverage: `test/bug-164-terminal-launcher-fallback.test.js` covers immediate failure, Windows failure after the old 100 ms boundary, Linux failure after that boundary, handshake-wait cancellation, temporary-listener cleanup, successful fallback, and exhaustion of every verifiable candidate without false activation. `test/bug-049-terminal-session-lifecycle.test.js` verifies that Windows activation never attempts the removed `cmd.exe` fallback, with or without recovery journaling.
...
- Regression coverage: `test/bug-201-macos-browser-focus.test.js` covers fresh process-tree discovery, exact PID, start-time, and bundle matching for Chrome, Firefox, Edge, and Brave, AppKit activation, removal of generic application-name activation, and fail-closed behavior for unknown, empty, or generation-less process sets. `test/bug-347-browser-focus-serialization.test.js` covers Focus lifecycle serialization.
...
- Regression coverage: `test/bug-079-send-unload-persistence.test.js` covers unsent edits without an unlocked workspace write, delayed stale writers, canceled/repeated unloads, concurrent remote tabs, queued deletion tombstones, focused header edits, storage failure, replay, and unload-handler registration; `test/bug-079-electron-quit-persistence.test.js` proves failed preflight leaves the renderer and backend intact and successful preflight precedes destructive cleanup; `test/bug-211-send-tab-concurrency.test.js` keeps cross-window merge and tombstone behavior covered.
...
- Regression coverage: `test/bug-087-keyboard-tabs.test.js` verifies static sidebar tab stops, keyboard activation/wrapping/focus behavior for both orientations, nested-control isolation, generated Send tab ARIA/roving state, distinct close labels, keyboard-close focus restoration, sibling tab/close ownership and hit areas, the labelled tabpanel, and the native add button.
...
- Regression coverage: `test/bug-142-curl-parser.test.js` covers mixed repeated data options including leading, trailing, and intervening empty parts; all representable URL-encoded forms and curl's space encoding; quoted Windows paths; lowercase Content-Type; Unicode Basic auth; literal `--data-raw @...`; safe rejection of file-backed operands; and paste-handler error/empty-body behavior.
