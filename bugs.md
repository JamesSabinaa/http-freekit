...
- Resolution: Update checks now use one owned in-flight state instead of a shared mutable boolean. Overlapping callers join the same upstream check, manual intent only promotes that state, a manual caller joining after the checking event receives feedback, and terminal attribution remains with the operation until its promise settles. Requests arriving after a terminal event wait for settlement and start a fresh check, preventing coalesced stale results from consuming a new request. Ambiguous shared updater errors are classified after the check promise settles, and lifecycle generations prevent queued or late check work and event/IPC listeners from crossing stop or re-initialization.
- Regression coverage: `test/bug-085-updater-check-attribution.test.js` covers automatic-to-manual promotion, protection from automatic demotion, a single attributed rejection, a request in the result/settlement gap, attribution reset for a later scheduled check, an overlapping install/check error, cancellation of queued checks on stop, and listener/late-settlement isolation across re-init.
