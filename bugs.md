...
- Status: **Fixed**.
- Resolution: Cleared pending-request tombstones now use a monotonic one-hour expiry and are capped at the 10,000-row traffic-log limit. Each proxy pending lifecycle also carries non-serialized lifecycle and clear-generation tokens, so a cleared completion remains suppressed even after its per-ID tombstone expires or is evicted. Append-style lifecycle events and WebSocket frames enforce the same generation, preventing frames from a cleared open connection from becoming orphaned records. Backend and renderer capacity trimming evict WebSocket child frames before base rows, keeping a high-volume open connection inspectable. Overlapping reuse of an ID selects and completes decisions by the original timestamp and route without consuming a newer lifecycle. Post-clear requests still surface consistently if ordinary traffic-log eviction removes their pending row, while normal retained completions clean up only their own lifecycle.
