...
- Status: **Fixed**.
- Resolution: Request-only, response-only, and combined request/response mock breakpoints now share the normal origin-forwarding pipelines for plain H1, intercepted H1, native H2, and H1-on-H2. Combined actions pause the edited request, forward it once, then pause and edit the real upstream response. Every route keeps the exchange in one pending traffic lifecycle instead of creating duplicate rows.
...
- Status: **Fixed**.
- Resolution: Mock rule validation is now shared by create, update, import, group, and persistence-loading paths. It validates supported action, transform-mode, and pre-step values plus their execution-sensitive fields, including HTTP method tokens, request/response headers, required action targets, and legacy response fields. Imports reject non-boolean `enabled` values instead of coercing them while preserving explicit `false` and defaulting omissions to `true`. Startup removes malformed leaves at every group depth and persists the sanitized tree, while runtime flattening and matcher evaluation independently fail closed if invalid data bypasses those boundaries.
...
- Status: **Fixed**.
- Resolution: Transform rules now forward the request after applying configured method, URL, header, and body changes, then apply status, header, and body transformations to the upstream response. Supported content encodings are decoded before body transforms and removed when plain transformed bytes are emitted, while intercepted secure clients can rewrite to HTTP as well as HTTPS origins. Legacy response-only transforms remain supported, and timeout rules keep the matched downstream request pending without contacting an origin before settling the capture if the client disconnects. The behavior is shared across plain HTTP, intercepted HTTPS, native H2, and H1-on-H2 routes.
...
- Status: **Fixed**.
- Resolution: H1 breakpoint URL edits now select the outbound HTTP or HTTPS transport from the rewritten URL and synchronize the request's Host header with its rewritten authority after all breakpoint edits. Single-phase and combined request/response actions share this forwarding path. Native H2 also synchronizes both outbound and captured Host data after simultaneous URL and header edits. Regression coverage verifies HTTP→HTTPS, HTTPS→HTTP in both intercepted H1 implementations, stale edited Host replacement, complete captures, and configured upstream-proxy routing.
...
- Status: **Fixed**.
- Resolution: The shared Content-Length setter now removes existing Content-Length, Transfer-Encoding, and Trailer fields case-insensitively before applying the edited body's exact length. Request and response body edits discard trailers associated with the old body, while unedited trailers remain preserved; when an H2 request with a known length and trailers falls back to H1, the proxy removes Content-Length before writing and uses valid chunked trailer framing. H1 conversions also remove stale Trailer declarations when no trailing fields arrived. Regression tests cover mixed-case chunked edits and both H2-to-H1 fallback edges.
...
- Status: **Fixed**.
- Resolution: Send now returns reversible UTF-8 text only when every response byte is valid UTF-8; all other bodies are returned as base64 data URIs with a strictly validated, normalized media type plus explicit bodyEncoding and byte-size metadata. The Send viewer and synthetic traffic entries retain that encoding, use the original wire byte count, decode byte-oriented views only when base64 provenance is present, and constrain and escape image preview sources.
...
- Status: **Fixed**.
- Resolution: WebSocket capture now retains RSV1 across complete and fragmented messages, parses the negotiated permessage-deflate direction and context parameters, and inflates text and binary payloads for display without changing relayed bytes. Inflation runs asynchronously behind bounded per-direction capture queues after raw relay writes, output is capped, and context takeover copies only its small sliding window instead of retaining decoded-message backing buffers. The complete handshake and connection lifecycle is registered before dialing upstream, downstream closure aborts pending handshakes, abrupt connection closure tears down both relayed sockets, and proxy shutdown waits for queued capture and final connection updates. Invalid reserved-bit use disables only capture, and undecodable compressed messages show an explicit marker instead of corrupt UTF-8.
...
- Evidence: ordinary traffic now enters guarded H1 and H2 streaming exchanges that relay request and response chunks with backpressure while a bounded collector records what fits. Body-dependent mocks, transforms, and breakpoints retain the buffered path. The relay preserves direct H2 negotiation, H1 fallback, upstream-proxy routing, informational responses, cancellation, and late trailers. `test/proxy/core/streaming-proxy.test.js` covers SSE-style responses, streaming uploads, H1-to-H2, H2-to-H2/gRPC, and H2-to-H1 bidirectional traffic; the body-limit regression verifies that capture truncation no longer rejects pass-through uploads.
...
- Reproduction: fixed; the origin and client timing assertions in `test/proxy/core/streaming-proxy.test.js` fail if either direction is buffered until `end`.
...
- Evidence: CA-pair validation parses the encoded forge serial and requires a nonzero value with a clear ASN.1 sign bit. `test/certificates/positive-certificate-serial.test.js` covers safe generation, persisted negative-serial migration, exact-thumbprint Windows replacement, install-before-delete ordering, and both add and cleanup failure paths.
...
- Status: **Fixed**.
- Resolution: Breakpoint create, update, and restore now require every matcher to satisfy the complete matcher contract. Resume payloads reject invalid URLs, methods, headers, non-final response statuses, and non-string bodies before releasing a pending request; HTTP/2 responses also sanitize connection-specific headers before writing, preventing malformed state from reaching Node's request/response writers.
...
- Status: **Fixed**.
- Resolution: HAR export and both import paths now preserve and validate explicit truncation, captured-size, and original-size metadata across round trips, including unknown original sizes and legacy captures without decoded-size metadata. Capture metadata covers interrupted uploads (including upstream resets and uploads rejected by a complete early response), prematurely closed streaming responses, large retained previews, omitted binary prefixes, and partial mock-file delivery without leaving pending records or misreporting unavailable bytes as captured content. The renderer marks incomplete exchanges and body views, warns that body searches cover retained bytes only, and refuses resend or mock creation when exact captured bodies are unavailable.
...
- Status: **Fixed**.
- Resolution: Startup preserves an expiring CA on every platform so a trusted root is never silently overwritten before replacement trust is available. Expiry and migration state are exposed 30 days ahead, and replacement requires an explicit, crash-safe renewal scheduled for the next restart. Windows trust cleanup retries and human client-migration acknowledgement use separate durable state; upgrades retain legacy recovery evidence until the acknowledgement marker's directory entry is durably synchronized, while redundant permission-hardening failures retain visible committed state without aborting startup. TLS settings refresh on entry and both UI and non-Windows boot warnings remain until the replacement CA has been installed in manually configured clients and acknowledged.
...
- Status: **Fixed**.
- Resolution: Chromium-family traffic is no longer discarded by an implicit host/path denylist. Account, update, Safe Browsing, telemetry, Web Store, Google UI, and failure requests now reach the API, UI, MCP tools, and exports; only Safe Font hosts are suppressed when the user explicitly enables that setting. That decision is fixed through each pending request lifecycle, including breakpoint and WebSocket intermediate updates, child frames, and append-style fallbacks, so a filtered parent cannot produce visible orphan frames. Breakpoint client disconnects retain the latest complete lifecycle record, allowing their terminal update to merge into the pending row or safely stand alone after normal log eviction, with duration measured from the original request start. Shutdown clears residual proxy decisions, preventing setting changes from leaving ghost rows, unmatched completions, or retained lifecycle state.
...
- Status: **Fixed**.
- Resolution: `get_request_detail` now traverses only bounded data descriptors, omitting accessors and excess metadata without proportional intermediate allocation, and returns explicit request, response, or original-request body pages. Complete legacy body fields remain intact whenever the wire-level MCP response fits; oversized defaults use explicitly named previews. Clients can repeat calls with a schema-matched safe UTF-16 code-unit offset and a maximum 32 KiB page size to retrieve every retained code unit, including production boxed binary/truncated original bodies, while each serialized MCP tool response remains capped at 512 KiB.
...
- Resolution: Newly generated root and per-host certificates now begin their validity five minutes before generation, using the same clock-skew tolerance as persisted-CA validation. Both expire exactly 365 days after their encoded generation time, at X.509's one-second precision, using epoch arithmetic so DST and leap-day boundaries cannot shorten or extend their lifetime.
...
- Status: **Fixed**.
- Resolution: Cleared pending-request tombstones now use a monotonic one-hour expiry and are capped at the 10,000-row traffic-log limit. Each proxy pending lifecycle carries non-serialized lifecycle and clear-generation tokens plus an opaque wire correlation ID, so a cleared completion remains suppressed even after its per-ID tombstone expires or is evicted. Proxy decisions, API row replacement, renderer updates, and WebSocket parent/frame association use that correlation ID, keeping overlapping reused request IDs independent even when timestamps match or a route is transformed. Legacy events without correlation IDs retain timestamp, route, and FIFO-compatible fallback matching, with primitive legacy decisions acting as FIFO barriers ahead of newer correlated records. Append-style lifecycle events and WebSocket frames enforce the same generation, preventing frames from a cleared open connection from becoming orphaned records. Backend and renderer capacity trimming use in-place fast paths that evict WebSocket child frames before base rows, keeping a high-volume open connection inspectable without per-frame full-array copies. JSON and HAR imports apply the same relationship-aware cap policy; collision remapping keys correlated children by parent ID and lifecycle, ambiguous legacy children deterministically bind to the first imported parent, and mixed-version children inherit the selected correlated lifecycle. Import validation rejects frames without a matching imported or retained WebSocket parent and validates nested connection metadata. Renderer frame indexes namespace legacy and lifecycle-aware keys so crafted legacy IDs cannot alias correlated connections, and remote endpoint values are escaped before detail rendering. The renderer recognizes both `ws` and secure `wss` parents for frame expansion, badges, row styling, and connected details, including WSS security, TLS, cipher, and remote metadata; pending, rejected, and failed handshakes retain the same available connection diagnostics. Post-clear requests still surface consistently if ordinary traffic-log eviction removes their pending row, while normal retained completions clean up only their own lifecycle.
...
- Status: **Fixed**.
- Resolution: Runtime browser, Docker, JVM, ADB, terminal, Electron, and Windows System Proxy discovery and lifecycle commands now run asynchronously. Startup awaits stale System Proxy recovery before exposing interceptor operations, while macOS bundle metadata, registry queries, policy and owner discovery, activation, WinINet notification, recovery, and restoration yield to other event-loop work.
...
- Status: **Fixed**.
- Resolution: new global-proxy fallback sessions are explicitly HTTP-only and no longer stage or request installation of a persistent user CA. Recovery journal version 6 distinguishes these safe sessions from version 1–5 global-proxy sessions; Stop opens Android Trusted credentials for a legacy session, returns a structured confirmation requirement, and retains durable ownership until the user confirms that the old HTTP FreeKit CA was removed. Replacement activation is refused while that marker exists, so it cannot rewrite the legacy record as a safe version-6 session. The renderer completes the two-step Stop confirmation flow, while HTTPS interception remains available through the companion VPN app.
- Regression coverage: `test/interceptors/android/android-ca-lifecycle.test.js` verifies that production fallback activation never issues an ADB push, persists the no-manual-CA marker, migrates legacy recovery ownership conservatively, performs no cleanup before confirmation, refuses replacement that would erase the marker, and wires the API/renderer handshake.
...
- Status: **Fixed**.
- Resolution: isolated and Global browser activation now shares a post-spawn stability check. Activation remains pending for a 500 ms startup confirmation window and removes its temporary listeners deterministically. Global activation rejects launch errors and exits/signals during that window; an isolated post-spawn exit first triggers inspection of its uniquely owned profile. Surviving profile descendants are adopted as the active lifecycle, confirmed-empty profiles are removed and rejected, and an unavailable inspection rejects while preserving active cleanup ownership so replacement cannot delete a possibly live browser profile. Later launcher errors apply the same descendant-safe lifecycle check. The manager publishes this conservative cleanup-failed state even though activation rejected, and the initiating UI also refreshes after activation errors so Stop remains visible without relying on its status WebSocket. Manager transition coalescing now also accounts for retained deactivation ownership, ensuring a later successful Stop broadcasts the final inactive state even if its pre-operation liveness inspection already marked the browser inactive.
- Regression coverage: `test/interceptors/browser/browser-spawn-confirmation.test.js` covers pending state before `spawn`, pending state after `spawn` but before stability confirmation, successful stable activation, pre-spawn errors, post-spawn exit/signal failures, managed-profile cleanup, surviving-descendant adoption, inspection-unknown ownership retention, its manager broadcast and final cross-client inactive broadcast, UI error refresh, later launcher errors, and temporary-listener cleanup for isolated and Global Chrome.
...
- Status: **Fixed**.
- Resolution: Fresh Terminal no longer guesses launcher health from a fixed 100 ms window. Each Windows, macOS, or Linux launcher remains watched for errors, signals, and nonzero exit until the existing durable shell PID/identity handshake succeeds; clean exits from client-style launchers remain valid while that handshake is pending. A failed candidate cancels its pending handshake wait, removes temporary listeners, stops the exact launcher, and advances to the next candidate, while the Windows Terminal fallback still waits for any unacknowledged child to close safely. The unverifiable standalone `cmd.exe` fallback was removed so activation cannot report success without a durable shell identity.
- Regression coverage: `test/interceptors/terminal/terminal-launcher-fallback.test.js` covers immediate failure, Windows failure after the old 100 ms boundary, Linux failure after that boundary, handshake-wait cancellation, temporary-listener cleanup, successful fallback, and exhaustion of every verifiable candidate without false activation. `test/interceptors/terminal/terminal-session-lifecycle.test.js` verifies that Windows activation never attempts the removed `cmd.exe` fallback, with or without recovery journaling.
...
- Regression coverage: `test/interceptors/system-proxy/system-proxy-winhttp.test.js` covers dual-store activation and exact restoration, machine scope, journal contents, independent external changes, failed-activation rollback, stale recovery, malformed-journal blocking, localized `netsh` output prefixes, command serialization, and post-write verification. Existing System Proxy suites use isolated WinHTTP fakes and continue covering WinINet recovery and retry behavior.
...
- Resolution: Android status refreshes now revalidate every definite or uncertain activation against the current authorized-device list. Global-proxy state is promoted only after an exact owned-proxy readback and becomes cleanup-only when another setting replaces it; companion state verifies package presence, a block-scoped live VPN state, and any exactly owned ADB reverse mapping. Sparse legacy VPN dumps fall back to detailed connectivity state, companion launches use per-device pending feedback until the VPN is observed, and confirmed/no-tunnel ownership plus confirmation history are journaled across intervening disconnect uncertainty and restarts. Cleanup preserves externally replaced reverse mappings; disconnects and ambiguous ADB results become explicit uncertainty instead of false active interception, while confirmed stopped sessions retain only necessary cleanup. Cleanup ownership remains available to Stop without being presented as live interception.
...
- Status: **Fixed**.
- Resolution: macOS Focus now takes a fresh managed-profile process-tree snapshot and fails closed if ownership or process generation cannot be revalidated. It passes only verified PID/start-time pairs to AppKit through JavaScript for Automation, resolves each exact `NSRunningApplication`, requires both the expected browser bundle identifier and matching launch time, and activates that specific application instance. The prior name-based AppleScript activation, which could select a normal-profile instance of the same browser, was removed. Manager serialization also prevents Focus from overlapping Stop or replacement activation.
- Regression coverage: `test/interceptors/browser/macos-browser-focus.test.js` covers fresh process-tree discovery, exact PID, start-time, and bundle matching for Chrome, Firefox, Edge, and Brave, AppKit activation, removal of generic application-name activation, and fail-closed behavior for unknown, empty, or generation-less process sets. `test/interceptors/browser/browser-focus-serialization.test.js` covers Focus lifecycle serialization.
...
- Regression coverage: `test/interceptors/browser/browser-focus-serialization.test.js` holds Focus and Stop at their operation boundaries and verifies that each rejects a concurrent operation for the same browser while unrelated browser operations remain independent.
...
- Resolution: Minimizing or closing the desktop window now cancels the native window action and hides the live window in the system tray, keeping the proxy and API session running. Wayland sessions use Electron's client-side Window Controls Overlay so its otherwise-unavailable minimize event reaches the same tray lifecycle, while other desktops retain their native frame. Tray/menu and second-instance restoration share a guarded show path that restores a minimized window before focusing it. Explicit Quit and restart still pass through the existing renderer/server cleanup gate; updater handoff may close only after its renderer preflight, including macOS's close-before-`before-quit` sequence, and canceled native unloads revoke that prepared allowance.
...
- Regression coverage: `test/build/readme-node-version.test.js` parses both declarations and requires their normalized minimum versions to remain identical.
...
- Regression coverage: `test/traffic/delete-traffic.test.js` covers authoritative API/search/export removal, reused WebSocket identities, ambiguity rejection, pending-completion suppression, renderer broadcast/response races, and renderer failure preservation.
...
- Resolution: Pin and unpin actions now mutate the exact authoritative traffic lifecycle through the management API and broadcast a revisioned state change to every renderer. Clear retains only valid server-pinned parent rows, returns their full authoritative snapshots, and broadcasts them atomically in ceiling-bounded chunks with pin-preserving deferred summaries and exact lifecycle hydration for individually oversized rows. When duplicate request IDs require the smallest fallback, compact wire identities retain each lifecycle discriminator rather than collapsing into ambiguous rows. Clear snapshots carry a pin-revision watermark, so mutations confirmed during chunk delivery or delayed hydration remain authoritative. Imported request and lifecycle IDs that cannot fit bounded broadcasts and safe management request targets are remapped consistently with their WebSocket frame parent references, and every final Clear fallback is size-checked before broadcast. One bounded generation token per pinned pending lifecycle keeps later HTTP completions and WebSocket frames attached across repeated Clears, while deleted retained lifecycles switch to expiring tombstones after completion. Traffic imports reject invalid pin types, frames cannot be pinned independently, and traffic dumps, request updates, Clear results, detail/search APIs, MCP, exports, and reloads all use server pin state.
- Regression coverage: `test/traffic/pinned-clear.test.js` covers exact reused-ID pinning and detail hydration, validation and ambiguity rejection, API/search/export persistence, ceiling-bounded chunked and final-fallback Clear delivery, compact duplicate-ID lifecycle fallbacks, pin mutations during compact chunk assembly, oversized request/lifecycle remapping with WebSocket parent consistency and usable management routes, unpin-and-clear behavior, repeated clears of pending requests, post-Clear WebSocket frames, bounded generation and deletion state, invalid imported/frame pins, authoritative renderer reconstruction, deferred Clear/detail upgrades that preserve later pin mutations, confirmation/failure behavior, stale-event rejection, and broadcast/response idempotency. Large-import coverage also verifies that deferred summaries preserve authoritative pin membership.
...
- Regression coverage: `test/send/send-tab-response.test.js` covers background completion, closed-tab completion, and the unchanged active-tab response path.
...
- Resolution: The renderer now journals every Send-tab upsert and deletion synchronously under a unique per-operation key, captures the active editor during `beforeunload`, and replays outstanding journals through the normal cross-window Web Lock. Journal resolution preserves concurrent tabs, makes deletion tombstones authoritative over stale upserts, and prevents delayed writers from replacing the newest unload snapshot. Header text fields update the captured model on every input, and a failed final journal write cancels navigation instead of silently losing edits. Electron now runs the same synchronous persistence preflight before closing the renderer, and only destroys the window, tray, and backend after it succeeds.
- Regression coverage: `test/send/send-unload-persistence.test.js` covers unsent edits without an unlocked workspace write, delayed stale writers, canceled/repeated unloads, concurrent remote tabs, queued deletion tombstones, focused header edits, storage failure, replay, and unload-handler registration; `test/desktop/electron-quit-persistence.test.js` proves failed preflight leaves the renderer and backend intact and successful preflight precedes destructive cleanup; `test/send/send-tab-concurrency.test.js` keeps cross-window merge and tombstone behavior covered.
...
- Regression coverage: `test/send/restored-multipart-files.test.js` covers restored metadata, live selected files, empty file rows, safe filename rendering in text and attributes, and keyboard access to the rendered warning/picker state.
...
- Resolution: The renderer now distinguishes staged mock drafts, changed open editors, and focused inline renames from unchanged or reverted work. Save All validates and stages a live editor before taking its upload snapshot, while Revert uses a work revision to reject delayed responses after any newer input. Reload, New Session, navigation, window close, Quit, Restart, and Restart to install all block on unsaved mock work; browsers use the standard leave-page prompt and Electron uses a native leave/stay prompt. Updater restarts are single-flight, run the same explicit renderer preflight, only bypass duplicate preparation after the native updater actually begins quitting, and release their pending state when a macOS native close is canceled or the installer fails synchronously. Monotonic updater event IDs distinguish a new same-version retry result from the listener/query replay of one result.
- Regression coverage: `test/mocking/mock-unload-guard.test.js`, `test/desktop/electron-unload-confirmation.test.js`, `test/mocking/mock-revert-transaction.test.js`, and `test/ui/toast-live-region.test.js` cover staged drafts, new, changed, reverted, and pre-blur editors, inline renames, Save All snapshot ordering, delayed Revert ownership, canceled/accepted unloads, Send persistence failure, Electron native confirmation and repeated cancellation recovery, updater restart single-flight/preflight and retry behavior, synchronous installer errors, handler registration, and Electron's combined quit preflight.
...
- Resolution: MCP remains enabled by default, but an explicit disabled state is now persisted transactionally and restored during startup. Toggle requests are serialized, validate a real boolean, verify the applied and rollback bridge states, and expose any runtime/persistence divergence as degraded. Failed transport cleanup remains owned for retry and prevents a disabled state from being persisted; process shutdown uses a best-effort cleanup path that also retries retained resources after an overlapping strict stop fails. SSE routes remain registered while disabled so status is explicit and later re-enabling remains available.
- Regression coverage: `test/mcp/mcp-enabled-persistence.test.js` covers the default, disabled, and re-enabled startup values; real settings-file round trips; toggle serialization; malformed input; verified persistence rollback; degraded-state reconciliation; retryable and overlapping shutdown transport cleanup; startup wiring; and the disabled SSE response. `test/mcp/mcp-toggle-renderer.test.js` covers the visible degraded state.
...
- Resolution: Direct stdio startup now records its input, output, and fatal-error callback as the desired transport configuration even when MCP starts disabled. Disabling still closes and detaches the active transport, while a successful re-enable creates a fresh server and stdio transport over the original streams before the toggle completes. The restored transport explicitly resumes stdin because the SDK pauses that stream when its previous transport closes. If failed cleanup retains a possibly live stdio owner, rollback exposes the degraded state without attaching a second reader; a later cleanup retry must release that owner first.
- Regression coverage: `test/mcp/mcp-stdio-toggle.test.js` covers an enabled stdio bridge across an off/on cycle, listener cleanup, stream preservation, restored JSON-RPC traffic and callback ownership, stdio configured during a persisted-disabled startup and attached on first enable, plus a real API rollback after failed stdio cleanup with one retained reader and one response.
...
- Resolution: Update checks now use one owned in-flight state instead of a shared mutable boolean. Overlapping callers join the same upstream check, manual intent only promotes that state, a manual caller joining after the checking event receives feedback, and terminal attribution remains with the operation until its promise settles. Requests arriving after a terminal event wait for settlement and start a fresh check, preventing coalesced stale results from consuming a new request. Checks, prompts/downloads, and installer handoffs have symmetric admission guards and lifecycle-owned completion state, so they cannot overlap at the updater layer, whose shared error event carries no operation metadata. Busy manual checks are queued with renderer feedback, stop/re-init drains old upstream checks and downloads before admitting new work, and re-init is refused while a native installer handoff may still emit. Install preflights retain their own generation callbacks, while stop/re-init invalidates queued work, pending dialogs, late status delivery, listeners, and IPC handlers.
- Regression coverage: `test/desktop/updater-check-attribution.test.js` covers automatic-to-manual promotion, protection from automatic demotion, a single attributed rejection, a request in the result/settlement gap, attribution reset for a later scheduled check, separately owned check/install errors, both forms of late old-preflight settlement, replacement of a pending prompt across re-init, cancellation of queued checks on stop, listener/late-settlement isolation, install/check admission, full download settlement, synchronous download setup failure, stale-download draining with queued feedback, and native-handoff re-init refusal.
...
- Resolution: The vertical sidebar and horizontal Send tablist now use roving tab stops synchronized with selection. Enter/Space, orientation-specific arrows, Home, and End activate and focus tabs with wrapping. Each generated Send tab and its native close button are sibling controls, so the close button retains independent semantics instead of becoming a presentational descendant of `role="tab"`. Close buttons include the tab position and request label, keyboard closure restores focus to the newly active tab, and clickable padding plus an inset focus ring keep the visual hit area aligned and unclipped. Generated Send tabs label their shared nested tabpanel, and New Tab is a native button.
- Regression coverage: `test/ui/keyboard-tabs.test.js` verifies static sidebar tab stops, keyboard activation/wrapping/focus behavior for both orientations, nested-control isolation, generated Send tab ARIA/roving state, distinct close labels, keyboard-close focus restoration, sibling tab/close ownership and hit areas, the labelled tabpanel, and the native add button.
...
- Status: **Fixed**.
- Resolution: Save All and every per-rule Save-to-server action now acquire the same synchronous in-flight lock before issuing a request. Reset, reorder, group, delete, import, automatic first-run default creation, traffic-derived mock creation, and breakpoint create/toggle/delete operations also claim or participate in shared collection-mutation ownership, so a save or cross-collection import cannot begin while an earlier server mutation is pending and a later mutation cannot alter its owned snapshot. Default creation only runs after a successful authoritative load and owns the empty collection before its POST. The lock marks the complete rule/group workspace busy, disables its form controls and mutating toolbar actions, and guards direct draft, collapse, group, drag, reset, delete, import, breakpoint, traffic-derived creation, default creation, and revert handlers. Each successful batch entry is still removed immediately, so a retry after a later failure cannot upload it again without allowing a newer local edit to be discarded by that cleanup.
- Regression coverage: `test/mocking/mock-save-all-lock.test.js` holds new- and existing-rule requests open and verifies duplicate per-rule saves, Save All overlap in both directions, Reset/collection ownership, lock release, whole-workspace and toolbar disabled state, Collapse All/Import protection, direct toggle/group-delete protection, incremental batch cleanup, and preservation of the uploaded snapshot. It also exercises real request ordering in both directions between Save All and traffic-derived mock creation, v2 Import and breakpoint mutation, and automatic default creation and traffic-derived creation. `test/mocking/mock-delete-draft.test.js` verifies deletion cannot mutate an in-flight save snapshot.
...
- Status: **Fixed**.
- Resolution: Every editor-navigation path now preserves the open edit first. Opening or adding another rule, expanding another card, collapsing the edited rule or its containing group, moving it into a collapsed group, and Collapse All save a valid edit as a local draft; if validation fails, the requested navigation or move is cancelled without changing the editor or expanded-card state. Collapsing a different card or unrelated group, and moving into an expanded group, no longer clears or stages the active editor.
- Regression coverage: `test/mocking/preserve-mock-editor.test.js` exercises valid and invalid edits through single-card collapse, containing-group collapse, collapsed-group moves, Collapse All, and expansion of a different rule, in addition to checking unrelated-group collapse, expanded-group moves, and direct Add/Edit navigation ownership.
...
- Regression coverage: `test/mocking/blank-matcher.test.js` verifies broad blanks never match, missing exact values fail shared/server validation, explicit empty exact values are accepted and match empty inputs, and the renderer initializes both exact-empty matcher types with `value: ''`.
...
- Regression coverage: `test/send/send-editor-startup.test.js` holds Monaco initialization pending on one tab, switches to another tab, changes its live request body, format, and body type, then verifies completion preserves those edits while refreshing the editor value, language, and visibility.
...
- Status: **Fixed**.
- Resolution: Repeated data options are joined in command order using curl's accumulated-length separator behavior, empty quoted arguments remain distinct tokens, and shell quoting preserves single-quoted plus non-special double-quoted backslashes. URL-encoded data now implements curl's `content`, `=content`, and `name=content` forms with UTF-8 percent encoding and `+` for spaces. File-backed data forms that cannot be reconstructed from clipboard text are rejected with a visible error instead of being sent literally. Header lookup remains case-insensitive and Basic auth encodes Unicode credentials as UTF-8.
- Regression coverage: `test/send/curl-parser.test.js` covers mixed repeated data options including leading, trailing, and intervening empty parts; all representable URL-encoded forms and curl's space encoding; quoted Windows paths; lowercase Content-Type; Unicode Basic auth; literal `--data-raw @...`; safe rejection of file-backed operands; and paste-handler error/empty-body behavior.
...
- Regression coverage: `test/desktop/updater-status-replay.test.js` drives the updater through a completed download followed by an up-to-date result and an error, verifying every later IPC snapshot still includes the downloaded version and that the renderer replays it.
...
- Resolution: Pause is now an authenticated server-owned capture state exposed through the management API, included in WebSocket initialization, and broadcast to every connected UI with a per-server session ID and monotonic revision. While paused, the API server neither retains nor broadcasts new intercepted traffic; Send traffic and explicit imports remain available. The API retention decision is propagated back into the proxy's exact lifecycle barrier, so requests and WebSocket parents rejected during Pause cannot surface completions or child frames after Resume, regardless of duration or traffic volume; a separate suppressed-completion callback preserves capture-independent proxy auto-rotation. Completion updates for pending rows captured before Pause are still applied so retained traffic cannot remain permanently pending. The renderer requests the authoritative state instead of filtering request events locally, ignores stale HTTP responses after newer cross-client broadcasts, resets ordering only for an initialized new server session, disables overlapping mutations, and exposes the toggle state to assistive technology.
...
- Resolution: OpenAPI uploads now parse `.yaml` and `.yml` selections with the packaged `js-yaml` browser build using its JSON-compatible schema, while `.json` selections retain strict JSON parsing. The parser rejects non-mapping document roots before prompting or uploading, and parser/load failures follow the existing error path without mutating the configured spec list.
...
- Resolution: OpenAPI enrichment now canonicalizes both configured and observed hosts—including bare and bracketed IPv6 forms—and requires exact, case-insensitive hostname equality, so lookalike prefixes and suffixes cannot inherit a spec. Path templates use a deterministic segment matcher instead of generated regular expressions: literal tokens are compared exactly and `{parameter}` tokens consume non-empty, slash-free gaps through forward-only searches. Punctuation, unmatched braces, adjacent parameters, and multiple parameters such as `{name}.{ext}` therefore retain OpenAPI semantics without regex injection or catastrophic backtracking.
