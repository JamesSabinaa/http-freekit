...
- Status: **Fixed**.
- Resolution: Mock rule validation is now shared by create, update, import, group, and persistence-loading paths. It validates supported action and pre-step types plus their execution-sensitive fields, including legacy response fields, and imports reject non-boolean `enabled` values instead of coercing them. Startup removes malformed leaves at every group depth and persists the sanitized tree, while runtime flattening and matcher evaluation independently fail closed if invalid data bypasses those boundaries.
