...
- Regression coverage: `test/bug-081-mock-unload-guard.test.js`, `test/bug-081-electron-unload-confirmation.test.js`, and `test/bug-256-mock-revert-transaction.test.js` cover staged drafts, new, changed, reverted, and pre-blur editors, inline renames, Save All snapshot ordering, delayed Revert ownership, canceled/accepted unloads, Send persistence failure, Electron native confirmation and cancellation, updater restart single-flight/preflight and retry behavior, synchronous installer errors, handler registration, and Electron's combined quit preflight.
