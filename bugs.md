...
- Status: **Fixed**.
- Resolution: Chromium-family traffic is no longer discarded by an implicit host/path denylist. Account, update, Safe Browsing, telemetry, Web Store, Google UI, and failure requests now reach the API, UI, MCP tools, and exports; only Safe Font hosts are suppressed when the user explicitly enables that setting. That decision is fixed through each pending request lifecycle, including breakpoint and WebSocket intermediate updates and append-style fallbacks. Breakpoint client disconnects retain the latest complete lifecycle record, so their terminal update can either merge into the pending row or safely stand alone after normal log eviction, with duration measured from the original request start. Shutdown clears residual proxy decisions, preventing setting changes from leaving ghost rows, unmatched completions, or retained lifecycle state.
