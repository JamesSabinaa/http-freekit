...
- Resolution: Pin and unpin actions now mutate the exact authoritative traffic lifecycle through the management API and broadcast a revisioned state change to every renderer. Clear retains only valid server-pinned parent rows, returns their full authoritative snapshots, and broadcasts them atomically in ceiling-bounded chunks with pin-preserving deferred summaries and exact lifecycle hydration for individually oversized rows. When duplicate request IDs require the smallest fallback, compact wire identities retain each lifecycle discriminator rather than collapsing into ambiguous rows. Delayed Clear/detail hydration upgrades only still-current deferred rows and preserves newer pin mutations. Imported request and lifecycle IDs that cannot fit bounded broadcasts and safe management request targets are remapped consistently with their WebSocket frame parent references, and every final Clear fallback is size-checked before broadcast. One bounded generation token per pinned pending lifecycle keeps later HTTP completions and WebSocket frames attached across repeated Clears, while deleted retained lifecycles switch to expiring tombstones after completion. Traffic imports reject invalid pin types, frames cannot be pinned independently, and traffic dumps, request updates, Clear results, detail/search APIs, MCP, exports, and reloads all use server pin state.
- Regression coverage: `test/bug-076-pinned-clear.test.js` covers exact reused-ID pinning and detail hydration, validation and ambiguity rejection, API/search/export persistence, ceiling-bounded chunked and final-fallback Clear delivery, compact duplicate-ID lifecycle fallbacks, oversized request/lifecycle remapping with WebSocket parent consistency and usable management routes, unpin-and-clear behavior, repeated clears of pending requests, post-Clear WebSocket frames, bounded generation and deletion state, invalid imported/frame pins, authoritative renderer reconstruction, deferred Clear/detail upgrades that preserve later pin mutations, confirmation/failure behavior, stale-event rejection, and broadcast/response idempotency. Large-import coverage also verifies that deferred summaries preserve authoritative pin membership.
