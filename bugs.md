...
- Status: **Fixed**.
- Resolution: Cleared pending-request tombstones now use a monotonic one-hour expiry and are capped at the 10,000-row traffic-log limit. Each proxy pending lifecycle also carries a non-serialized clear-generation token, so a cleared completion remains suppressed even after its per-ID tombstone expires or is evicted. Append-style lifecycle events and WebSocket frames inherit and enforce the same token, preventing frames from a cleared open connection from becoming orphaned records. Post-clear requests carry the current generation and still surface consistently if ordinary traffic-log eviction removes their pending row. Normal retained completions and ID reuse remove their tombstones immediately.
