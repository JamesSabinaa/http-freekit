...
- Resolution: Pause is now an authenticated server-owned capture state exposed through the management API, included in WebSocket initialization, and broadcast to every connected UI with a monotonic revision. While paused, the API server neither retains nor broadcasts new intercepted traffic; Send traffic and explicit imports remain available. The API retention decision is propagated back into the proxy's exact lifecycle barrier, so requests and WebSocket parents rejected during Pause cannot surface completions or child frames after Resume, regardless of duration or traffic volume. Completion updates for pending rows captured before Pause are still applied so retained traffic cannot remain permanently pending. The renderer requests the authoritative state instead of filtering request events locally, ignores stale HTTP responses after newer cross-client broadcasts, disables overlapping mutations, and exposes the toggle state to assistive technology.
