...
test('application wires the API shutdown route to the centralized cleanup path', () => {
  ...
  assert.match(
    source,
    /await mcpBridge\.stop\(\{ bestEffort: true \}\);[\s\S]*?await interceptors\.deactivateAll\(\{ onProgress: notifyDesktopShutdownProgress \}\);[\s\S]*?await proxy\.stop\(\);[\s\S]*?await api\.stop\(\)/
  );
});
